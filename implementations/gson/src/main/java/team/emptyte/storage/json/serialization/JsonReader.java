...
public class JsonReader implements Reader<JsonObject> {
  ...
  public JsonReader(final @NotNull JsonObject jsonObject) {
  ...
  public <T> @Nullable T readObject(
    ...
    return typeSerializer.deserialize(new JsonReader(element.getAsJsonObject()));
  }
  ...
  public @Nullable <K, V> Map<K, V> readMap(
    ...
    final @NotNull TypeSerializer<V, JsonObject> typeSerializer
  ) {
    final var element = this.jsonObject.get(field);
    if (element == null) {
      return null;
    ...
    for (final var arrayElement : array) {
      final var value = typeSerializer.deserialize(new JsonReader(arrayElement.getAsJsonObject()));
  ...
  public <T, C extends Collection<T>> @Nullable C readCollection(
    ...
    for (final var element : array) {
      final var object = typeSerializer.deserialize(new JsonReader(element.getAsJsonObject()));
      objects.add(object);
    }
    return objects;
  }
